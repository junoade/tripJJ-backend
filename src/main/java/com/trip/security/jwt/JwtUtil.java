...
public class JwtUtil {
	...
	/**
	 * Generate JWT jwt 토큰의 구성 : header(alg, typ) + payload(data) + signature(시크릿키)
	 * 
	 * @param userId        사용자 ID
	...
	private String createJwt(String userId, String subject, Long expiredMillis) {
		...
		claims.setSubject(subject).setIssuedAt(new Date())
		...
		return Jwts.builder() // io.jsonwebtoken.impl 필요
				...
				.signWith(key(), SignatureAlgorithm.HS256).compact(); // 직렬화 처리
	}
